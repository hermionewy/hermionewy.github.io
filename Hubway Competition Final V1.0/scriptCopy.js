...
function nestDataById (data){
    ...
    //        .call(timeseries);
    var info = d3.select('#info').select('p')
      .attr('class','info')
      .append('text');
...
//        console.log(selectedBike);
        var bikeNum = selectedBike.length;
          ...
          info.text('There are '+ bikeNum +' Hubway bikes working '+ range[0].toFixed(2)+' ~ '+ range[1].toFixed(2) +' hours per day in 2016.');
    ...
    bikeTrend.on('selectBike', function(d){
        ...
        function drawCircles(trips){
         ...
         // for each station append a circle
            ...
            var circle = L.circle(idToLocation.get(d.key),{color: '#A5C0D1',fillColor: '#f03',fillOpacity: 0.1, radius: scaleR(+d.value)})
