  ...
  //duration/(366*60)
    ...
    _thresholds = d3.range(0,maxDuration,0.2);
		...
		var svgEnter = svg.enter()
			...
			var rect = plotEnter.selectAll('rect').data(dayBins);
			...
			plotEnter.append('line').attr('class','meanLine').style('stroke-dasharray',('2,2')).style('stroke','#65ba69');
			...
			var median = (arr.length%2)? arr[(arr.length+1)/2].duration : arr[arr.length/2].duration;
			var mean = d3.mean(arr,function(d){ return d.duration });
			console.log(median);
			console.log(mean);
